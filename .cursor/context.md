...
- Discord-style Activity Feed (friends/achievements/milestones/patches tabs) — initial cap 25 events (`ACTIVITY_INITIAL_LIMIT` in `Home.tsx`, paginates up to `ACTIVITY_MAX_LIMIT` 100); "View full feed" footer button + "Open community →" header link both navigate to Community page. Sits side-by-side with "Hot this week" in the `.bi-home-duo` 2-col grid ≥981px (stacks below; ActivityFeed goes full-width when trending has no data)
...
- Forums table (channels) — LIVE (GET /forums/categories). Forums proper: post types, image uploads, FTS, polls, reactions (any Unicode emoji + synced guild custom emoji via lazy `EmojiPicker` — ONE flat Discord-style scroll panel: search, localStorage "Frequently used" MRU, guild section, 8 unicode categories under sticky headers, no tabs; chips under a post render only reactions that exist (count > 0, none = no row); the legacy five keys render on old posts but are no longer offered; `c:<snowflake>` keys resolved through the `guild_emojis` cache), game tagging (`forum_threads.app_id`) with feed/search `?appId` filter + clickable GameChip, OP badge on the thread author's replies. Post actions = Discord-style hover bar top-right of each post (`PostActionBar.tsx`: react/share/quote/edit/delete/report; hover or focus-within reveal, always-visible dimmed on touch; reaction chips stay in the footer). Editor (`forumEditor.tsx` MarkdownEditor, shared by composer + replies) is contextual: toolbar on focus (always on touch), persistent "+" attach, Ctrl+B/I/E/K + Ctrl+Shift+X, Enter continues lists, paste/drag images auto-upload, paste-URL-over-selection links it. Anchored menus (share, emoji picker) render via `PortalPopover` (portal to body, z 300; bottom sheet on small screens)
...
- News pipeline (v3.2, June 2026 — PRs #61-#64):
  ...
  - **Two AI gates, both AI-authoritative** (2026-07): gate 1 `offTopic` — "is the core subject gaming?" (parks `off_topic_ai` / sweep `off_topic_ai_sweep`); gate 2 `crewFit` — "does THIS crew plausibly care?", judged against the injected crew context (playtime/library/wishlist/genres; parks `crew_irrelevant` / sweep `crew_irrelevant_sweep`). When-uncertain-keep bias on both. One-shot post-deploy sweep re-runs per `SWEEP_VERSION` bump (`newsOffTopicSweep.ts`, v3). Health endpoint surfaces a `crewIrrelevant` counter. See `DESIGN_NOTES.md` → News.
