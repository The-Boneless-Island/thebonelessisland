...
| **Admin role** | The Discord role (`ADMIN_ROLE_NAME` / `server_settings.admin_role_name`) that gates admin/mod endpoints (`requireAdminRole`). Renamed from "Parent role" — that name was leftover from an early test-server role and had no relation to actual admin-level access. |
